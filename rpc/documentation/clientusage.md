...
var certificateFile = filepath.Join(dcrutil.AppDataDir("eacrwallet", false), "rpc.cert")
...
```bash
$ protoc -I/path/to/eacrwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/eacrwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by eacrwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.eacrwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\eacrwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by eacrwallet).
...
if (os.platform() == 'win32') {
  ...
  certPath = path.join(process.env.HOME, '.eacrwallet', 'rpc.cert');
}
...
def main():
    ...
    if platform.system() == 'Windows':
        ...
        cert_file_path = os.path.join(os.environ['HOME'], '.eacrwallet', 'rpc.cert')
